...
// Маршрут для уменьшения остатков товара в магазине
router.post('/decrease_remains', async (request, response) => {
    ...
    try {
        ...
        // Если записи не найдено, откатываем транзакцию и возвращается ошибка 404 (Not Found)
        if (current_result.rowCount === 0) {
        ...
        // Проверка, что после уменьшения остатков они не станут отрицательными
        if (previous_shelf < shelf_quantity || previous_order < order_quantity) {
            await pool.query('ROLLBACK');
            return response.status(400).send('Остатки станут отрицательными после выполнения операции');
        }

        // SQL-запрос для уменьшения остатков
        ...
        // Отправка информации о действии в историю - уменьшение остатков
        await send_action_to_history({
            action: 'decrease_remains',
            PLU,
            store_name,
        ...
        response.status(500).send('Ошибка при уменьшении остатка');
    }
});
